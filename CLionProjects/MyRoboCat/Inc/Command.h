...
#ifndef MYROBOCAT_COMMAND_H
#define MYROBOCAT_COMMAND_H
...
public:
    ...
    static std::shared_ptr<Command> StaticReadAndCreate(InputMemoryBitStream &inInputStream);
...
public:
    AttackCommand() : mTargetNetId(0) {
        mCommandType = CM_ATTACK;
    }

    static std::shared_ptr<AttackCommand> StaticCreate(uint32_t inMyNetId, uint32_t inTargetNetId);
...
public:
    ...
    static std::shared_ptr<MoveCommand> StaticCreate(uint32_t inMyNetId, const Vector3 &inTarget);
